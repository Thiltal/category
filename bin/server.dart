...
void main(){
  ...
  middle = sessionMiddleware(new SimpleSessionStore());
  myRouter = router();
  shelf.Handler handler = new shelf.Cascade().add(staticHandler).add(myRouter.handler).handler;
  route("/api/app_state", appState);
  routeGet("/api/app_state", appState);
  routeGet("/api/test_error", (RequestContext context){
//    double a = 5;
//    context.write(a.toString());
//    context.close();
  });
  route("/api/save_user", saveUser);
  route("/api/login", login);
  route("/api/get_problems", getProblems);
  route("/api/solve_problem", solveProblem);
  myRouter.get("/api/test", (shelf.Request request){
     return new shelf.Response.ok("test passed");
  });
  route("/api/backup", (RequestContext context){
    context.write(allData());
  });
...
void routeGet(String path, DHandler controller){
  myRouter.get(path, (shelf.Request request){
    StreamController innerController = new StreamController();
    Stream<List<int>> out = innerController.stream;
